...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    Parameters:
        ...
        netG (str) -- the architecture's name: resnet_9blocks | resnet_6blocks | unet_256 | unet_128 | custom
    ...
    if netG == 'dvf':
        net = GeneratorUNetDVF(in_channels=input_nc, out_channels=output_nc)
    else:
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netD == 'dvf':
        net = DiscriminatorDvf(in_channels = input_nc)
    else:
...
class AdaIN_diff3D(nn.Module):
    def __init__(self, num_channel, out_size):
        super(AdaIN_diff3D,self).__init__()
        
        self.norm = nn.InstanceNorm3d(num_features=out_size, affine=False) # (x - mean(x))/stdev(x)
        self.fc2 = nn.Linear(1, num_channel*2)
        self.c1 = nn.Conv1d(num_channel*2, num_channel, 1)
        self.c2 = nn.Conv1d(num_channel*2, num_channel, 1)
        
    def forward(self, x, surface):
        # Normalize feature
        normalized = self.norm(x)
        
        # "Transform the scalar"
        h2 = self.fc2(surface)
        h2 = h2.view(h2.size(0), h2.size(1), 1)
        
        # Calculate Gamma and reshape to the good size
        gamma = self.c1(h2)
        gamma = gamma.view(gamma.size(0), gamma.size(1), 1, 1, 1)
        
        # Calculate Beta and reshape to the good size
        beta = self.c2(h2)
...
class GeneratorUNetDVF(nn.Module):
    def __init__(self, in_channels=1, out_channels=1):
        super(GeneratorUNetDVF, self).__init__()

        self.down1 = UNetDown(in_channels, 64, normalize=False)
        self.down2 = UNetDown(64, 128)
        self.down3 = UNetDown(128, 256)
        self.down4 = UNetDown(256, 512)
        self.adafc = AdaIN_diff3D(512, 512)
    ...
    def forward(self, x, surface):
        ...
        d3 = self.down3(d2)
        d4 = self.down4(d3)
        ad4 = self.adafc(d4, surface)
        m1 = self.mid1(ad4, ad4)
        m2 = self.mid2(m1, m1)
        m3 = self.mid3(m2, m2)
        m4 = self.mid4(m3, m3)
        u1 = self.up1(m4, d3)
...
class DiscriminatorDvf(nn.Module):
    def __init__(self, in_channels=1):
        super(DiscriminatorDvf, self).__init__()
    ...
    def forward(self, img_A, img_B, dvf):
        # Concatenate image and condition image by channels to produce input

        tmp = torch.cat((img_A, img_B), 1)
        img_input = torch.cat((tmp, dvf), 1)
